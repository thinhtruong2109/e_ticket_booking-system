...
const navItems = [
  ...
  { label: 'Promo Codes', path: '/admin/promo-codes', icon: <LocalOffer /> },
  { label: 'Wallets', path: '/admin/wallets', icon: <AccountBalanceWallet /> },
  { label: 'Categories', path: '/admin/categories', icon: <Category /> },
  { label: 'Venues', path: '/admin/venues', icon: <LocationOn /> },
    ...
    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
      ...
      <Box sx={{ p: 2, display: 'flex', alignItems: 'center', gap: 1.5 }}>
        ...
        <Box sx={{ minWidth: 0 }}>
          ...
          <Chip label={getRoleLabel(user?.role)} size="small" color="error" sx={{ height: 20, fontSize: '0.7rem' }} />
        </Box>
      </Box>
      <Divider />
        ...
        {isMobile && (
          <AppBar position="sticky" color="default" elevation={1}>
            <Toolbar>
              ...
              <Typography variant="h6" fontWeight={600}>Admin Portal</Typography>
            </Toolbar>
          </AppBar>
        )}
